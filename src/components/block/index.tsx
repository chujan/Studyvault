...
export const Block: FC<Props> = ({
  ...
  const blockStyle = StyleSheet.flatten([
    ...
    paddingVertical !== undefined && {paddingVertical: RS(paddingVertical)},
      ...
      {overlayColor ? (
        <TouchableOpacity
          ...
          style={[
            ...
            {backgroundColor: overlayColor, opacity: disabled ? 0.5 : 1},
          ]}
          onPress={() => {
            Keyboard.dismiss();
            if (onPress) {
    ...
    return (
      <View
        ...
        style={[
          {
            paddingTop: RS(insets.top),
            paddingBottom: RS(insets.bottom),
            flex: flex ?? 1,
            backgroundColor: bg,
          },
        ]}
  ...
  if (scroll) {
    return (
      <View
        testID={testID}
        style={{flex: 1, backgroundColor: bg, paddingTop: RS(insets.top)}}
        {...props}>
        <KeyboardAwareScrollView
          bottomOffset={50}
          contentContainerStyle={contentContainerStyle}
  ...
  return (
    <TouchableOpacity
      ...
      style={[blockStyle, {opacity: disabled ? 0.5 : 1}]}
      {...props}>
      {children}
    </TouchableOpacity>
  );
