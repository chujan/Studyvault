    ...
    <Block flex={1}>
      ...
      <Block style={[styles.header, { marginTop: insets.top }]} paddingHorizontal={RS(20)} paddingVertical={RS(16)}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: RS(16) }} contentContainerStyle={{ paddingHorizontal: RS(4) }}>
          <TouchableOpacity style={{ paddingHorizontal: RS(12), paddingVertical: RS(6), backgroundColor: palette.blue, borderRadius: RS(16), marginRight: RS(8) }}>
            <Text size={14} color={'white'}> {currentSubject} </Text>
          </TouchableOpacity>
        </ScrollView>
      </Block>
...
{currentInstruction && (
  <Block style={{ marginBottom: RS(12) }}>
    <Text size={14} style={{ fontWeight: '600', marginBottom: RS(4) }}>
      Instruction:
    </Text>
    <Text size={14}>{currentInstruction}</Text>
  </Block>
)}
      ...
      {showSubmitPopup && (
        <Block style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.4)', alignItems: 'center', justifyContent: 'center' }}>
          <Block style={{ width: '85%', backgroundColor: '#FFF', borderRadius: RS(16), padding: RS(20) }}>
            ...
            <Text size={14} style={{ fontFamily: family.Medium, marginBottom: RS(12) }}>Are you sure you want to submit test?</Text>
          ...
          <Block style={{ width: '90%', backgroundColor: '#fff', borderRadius: RS(16), padding: RS(20) }}>
            ...
            <Block row wrap="wrap" justify="space-between">
              {['7','8','9','/','4','5','6','*','1','2','3','-','0','.','=','+','C'].map(btn => (
                <TouchableOpacity key={btn} onPress={() => handleCalcPress(btn)} style={{ width: '22%', height: RS(50), marginBottom: RS(12), backgroundColor: '#f2f2f2', alignItems: 'center', justifyContent: 'center', borderRadius: RS(8) }}>
